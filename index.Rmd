...
display = t[,list(
  ...
  `ส่วนต่าง` = format(extrapolate,big.mark = ",")
...
- จากกราฟ 5.4 กราฟเป็นเส้นโค้งถึงตำแหน่งที่ 300
...
>คาดการณ์ว่ามีการสาดกระสุน **อย่างๆน้อยๆ** 8 แสนไปจนถึง 1 ล้านนัดทั่วประเทศ ตีมูลค่าได้นับพันล้านบาท> นอกจากพรรคก้าวไกลและพรรคเพื่อไทยแล้ว อีก 4 พรรคที่เหลือมีรูปแบบที่ชัดเจนคือ มักจะได้ที่นั่ง สส ในเขตต้องสงสัยกันทั้งสิ้น มีความสัมพันธ์ระหว่างคะแนนของพรรคเล็กและโอกาสที่จะชนะในเขตอย่างชัดเจน แต่อาจจะมีกลยุทธ์การยิงที่แตกต่างกันออก

>แต่อย่างไรก็ตามไม่ได้แปลว่าจะมีการสาดกระสุนกันเพียง 800,000-1,000,000 นัด แต่อาจจะยังมีกระสุนอีกมากที่เรายังไม่พบ อาจจะที่ยิงแล้วไม่เข้าเป้า หรือมือปืนรับจ้างเก็บกระสุนไปโดยไม่บอกนายแต่อ้างว่ายิงไปแล้ว ซึ่งตัวเลขอาจจะมากกว่านี้เป็นเท่าตัว เรายังไม่อาจทราบได้จากร่องลอยเหล่านี้


> พรรคก้าวไกลเป็นพรรคที่ใสสะอาดที่สุดในทั้งหมด 6 พรรคใหญ่ ไม่พบความสัมพันธ์ระหว่างคะแนนพรรคเล็กและเสียงคะแนน สส เขต

>แต่ก็ไม่สามารถสรุปได้ว่าพรรคก้าวไกลไม่มีการซื้อเสียง เพียงแต่ว่าเรายังไม่พบกระสุนเท่านั้นเอง


> พรรคเพื่อไทยเป็นพรรคที่มีผลลัพธ์ที่สำหรับผลน่าแปลกใจที่สุดในทั้ง 6 พรรคจากข้อมูลชี้ให้เห็นว่า พรรคเพื่อไทยมีโอกาสที่จะสาดกระสูนแทบทุกเขตการเลือกตั้ง เราปฏิเสธไม่ได้เลยว่าคะแนนพรรคเล็กมีความสัมพันธ์กับเลขผู้สมัครของพรรคเพื่อไทยอย่างมีนัยยะสำคัญ
...
> เราายังสามารถประมานการณ์ คะแนนบัญชีรายชื่อที่แท้จริงหากไม่มีการสาดกระสุนเกิดขึ้น ได้ด้วยการเทียบบัญญัติยางค์จากตัวเลขคะแนนบัญชีรายชื่อของเขตที่ไม่ชนกับผู้สมัครพรรคใหญ่ และจำนวนเขต ซึ่งจะได้ออกมาว่ามีปลอกกระสุนราวๆ 873,678 นัด
...
v = result[no %in% c(1:6,8:9),list(
  seat = sum(mp_app_rank==1),
  vote = sum(zone_vote)
), by =list(party_name)]

######
v1 = result[no %in% c(1:6,8:15) & party_no %in% bigParty,list(
  cons_id,
  no
)] %>% unique

v = result[v1, on = list(cons_id,party_no = no)] 
v = v[,list(
  party_list_vote = sum(party_list_vote)
), by = list(party_no)]
...
v = allSuspect[index <= cut_off]
v[,ratio := party_list_vote/zone_vote ]
v = v[party_no %in% bigParty & is.finite(ratio)]
v = v[,list(party_name,party_list_vote,zone_vote,ratio)]
#v = v[,list(cnt = .N), by =list(party_name,province_name)]

v = result[v,suspect := !is.na(i.party_name),on = list(cons_id,party_id)]
v[is.na(suspect), suspect := FALSE]
v[,ratio := party_list_vote/zone_vote ]


v = v[party_no %in% bigParty,list(
  sum(party_list_vote)/sum(zone_vote)
),by =list(party_name,suspect)]

### KLaoklai ratio as standard

vv =result[party_no %in% bigParty,list(
  mean = sum(party_list_vote)/sum(zone_vote),
  sd = sd(party_list_vote/zone_vote,na.rm = 1)
),by = list(party_name)]
